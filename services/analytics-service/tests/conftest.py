...
async def client():
    ...
    db_url = os.getenv(
        "TEST_DATABASE_URL",
        "sqlite+aiosqlite:///./test_analytics.db",
    )
    using_sqlite = _is_sqlite(db_url)

    execution_options = {}
    if using_sqlite:
        execution_options["schema_translate_map"] = {"analytics_schema": None}
    ...
    async with engine.begin() as conn:
        if not using_sqlite:
            await conn.execute(text("CREATE SCHEMA IF NOT EXISTS analytics_schema"))
    ...
    async with engine.begin() as conn:
        if using_sqlite:
            await conn.execute(text("DELETE FROM campaign_metrics"))
            await conn.execute(text("DELETE FROM campaign_owners"))
        else:
            await conn.execute(text("DELETE FROM analytics_schema.campaign_metrics"))
            await conn.execute(text("DELETE FROM analytics_schema.campaign_owners"))
